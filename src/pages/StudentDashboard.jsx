      ...
      <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-10 py-6 border-b border-stone-100">
          <div>
            ...
            <p className="text-stone-400 text-[10px] font-black uppercase tracking-widest mt-1">Event Memories</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {event.galleryImages?.map((url, i) => (
              <div key={i} className="aspect-square rounded-[32px] overflow-hidden bg-stone-50 border border-stone-200 group">
                <img src={url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="Gallery" />
              </div>
            ))}
          </div>
        </div>
...
export default function StudentDashboard() {
  ...
  // ── FAIL-SAFE LINK PROTOCOL ──
  const handleRegister = (link) => {
    if (!link) {
      alert("Registration link is pending from Admin.");
      return;
    }
    const url = link.startsWith('http') ? link : `https://${link}`;
    window.open(url, '_blank');
    alert("Smart Clash Verified: You are eligible. Redirecting to form...");
  };
      ...
      <div className="max-w-7xl mx-auto px-6 mt-10">
        ...
        <div className="rounded-[56px] p-12 lg:p-16 mb-12 flex justify-between items-center text-white shadow-2xl relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #162d4a 100%)' }}>
          <div className="relative z-10">
            ...
            <p className="text-blue-200 text-lg mt-3 font-medium opacity-90">Smart Clash has matched {events.length} upcoming events to your profile.</p>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div className="flex gap-2 p-1.5 bg-white rounded-3xl border border-stone-200 shadow-sm">
            {['All', 'Tech Event', 'Cultural Event'].map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-6 py-3 rounded-2xl text-[10px] font-black transition-all uppercase tracking-widest ${filter === f ? 'text-white shadow-xl scale-105' : 'text-stone-400 hover:text-stone-600'}`} style={filter === f ? { background: 'linear-gradient(135deg, #1e3a5f, #162d4a)' } : {}}>{f}</button>
            ))}
          </div>
        </div>
              ...
              {filtered.map(ev => {
                ...
                return (
                  <div key={ev._id} className="bg-white rounded-[48px] border border-stone-200 shadow-sm overflow-hidden flex flex-col hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 group">
                    ...
                    {actualImage ? (
                      <img src={actualImage} className="w-full h-60 object-cover border-b group-hover:scale-105 transition-transform duration-700" alt="Brochure" />
                    ) : (
                      <div className="w-full h-60 bg-stone-50 flex items-center justify-center text-stone-200"><ImageIcon className="w-12 h-12" /></div>
                    )}
                    ...
                    <div className="p-10 flex flex-col flex-1">
                      ...
                      <button onClick={() => handleRegister(actualLink)} className="w-full py-5 rounded-[24px] text-white font-black text-[11px] tracking-[0.2em] shadow-xl uppercase transition-all hover:brightness-110 active:scale-95 flex items-center justify-center gap-2" style={{ background: 'linear-gradient(135deg, #1e3a5f, #162d4a)' }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
              {past.map(ev => (
                <div key={ev._id} onClick={() => setGallery(ev)} className="bg-white p-8 rounded-[40px] border border-stone-200 shadow-sm hover:shadow-2xl cursor-pointer transition-all text-center">
                  <div className="w-16 h-16 bg-stone-50 rounded-[20px] flex items-center justify-center mx-auto mb-6"><ImageIcon className="w-8 h-8 text-stone-300" /></div>
                  <h4 className="font-bold text-stone-800 text-xs line-clamp-1">{ev.title}</h4>
                  <p className="text-[10px] text-stone-400 font-black uppercase mt-2 tracking-widest">{ev.galleryImages?.length || 0} Photos</p>
                </div>
              ))}
            </div>
          </>
